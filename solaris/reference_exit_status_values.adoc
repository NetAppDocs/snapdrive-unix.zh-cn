...
管理员错误
 a| 
此密码文件的条目错误。使用 SnapDrive config delete` 命令删除此存储系统的登录条目。然后使用 ` _sn SnapDrive config setuser_name_` 命令重新输入登录信息。
...
管理员错误
 a| 
此密码文件没有此存储系统的条目。对需要运行 SnapDrive for SnapDrive 的每个存储系统运行 ` _UNIX config setusername filername_` 命令。然后重试此操作。
...
在禁用自动主机准备的情况下，您应使用 SnapDrive config prepare lun` 命令来准备主机以配置 LUN ，或者手动执行准备步骤。
